...
def cmd_model(args):
    ...
    elif selected_provider == "xgate":
        _model_flow_xgate(config, current_model)
...
def _model_flow_xgate(config, current_model=""):
    """xgate provider flow with optional TaskMarket/x402 wallet onboarding."""
    from hermes_cli.auth import _save_model_choice, deactivate_provider
    from hermes_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    current_base = get_env_value("XGATE_BASE_URL") or os.getenv("XGATE_BASE_URL", "") or "https://ai.xgate.run/v1"
    ...
    print("Configure xgate access.")
    print("Primary paid mode uses the TaskMarket wallet for x402 request signing.")
    print("Hermes will reuse ~/.taskmarket/keystore.json or run `taskmarket init` for you.")
    print("Helper/static header fallback remains available via XGATE_AUTH_HELPER_CMD or XGATE_REQUEST_HEADER_VALUE.")
    print()
    ...
    save_env_value("XGATE_BASE_URL", base_url.rstrip("/"))
    ...
    if isinstance(model, str):
        ...
        model = {}
    model["provider"] = "xgate"
    model["base_url"] = base_url.rstrip("/")
    cfg["model"] = model
    save_config(cfg)
    deactivate_provider()

    print(f"Default model set to: {model_name} (via xgate)")
